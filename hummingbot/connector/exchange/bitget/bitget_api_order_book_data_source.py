...
class BitgetAPIOrderBookDataSource(OrderBookTrackerDataSource):
        ...
        while True:
            try:
                ...
                if self._pong_response_event and not self._pong_response_event.is_set():
                    ...
                    raise IOError("The order book stream channel is unresponsive (pong response not received)")
    ...
    async def _subscribe_channels(self, ws: WSAssistant):
        try:
            ...
            for trading_pair in self._trading_pairs:
                instId = await self._connector.instrument_id_associated_to_pair(trading_pair=trading_pair)
    ...
    async def _request_order_book_snapshot(self, trading_pair: str) -> Dict[str, Any]:
        ...
        data = await rest_assistant.execute_request(
            url=web_utils.get_rest_url_for_endpoint(endpoint=CONSTANTS.QUERY_DEPTH_ENDPOINT),
            params=params,
            method=RESTMethod.GET,
            throttler_limit_id=CONSTANTS.QUERY_DEPTH_ENDPOINT,
        )
