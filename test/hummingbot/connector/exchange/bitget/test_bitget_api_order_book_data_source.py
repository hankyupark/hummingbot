...
class BitgetAPIOrderBookDataSourceTests(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.connector._set_trading_pair_symbol_map(
            bidict({f"{self.base_asset}{self.quote_asset}_SPBL": self.trading_pair}))
    ...
    # 스펙 문서(https://bitgetlimited.github.io/apidoc/en/spot/#depth-channel)에 따르면 asks, bids는 2개의 배열로 구성되어야 한다.
    # 하지만, 이 함수가 참조한 bitget_perpetual_api_order_book_data_source.py의 get_ws_snapshot_message()은 4개의 배열로 구성되어 있다.
    # 이유를 알 수 없어서, 우선 bitget_perpetual_api_order_book_data_source.py의 get_ws_snapshot_message()를 그대로 가져왔다.
    def get_ws_diff_msg(self) -> Dict:
        return {
            "action": "update",
            "arg": {
                "instType": CONSTANTS.INSTRUMENT_TYPE_SPOT_PUBLIC_CHANNEL,
    ...
    def test_get_new_order_book_successful(self, mock_api):
        endpoint = CONSTANTS.QUERY_DEPTH_ENDPOINT
    ...
    def test_listen_for_subscriptions_for_trading_pair(self, ws_connect_mock):
        local_base_asset = "BTC"
        local_quote_asset = "USDC"
        local_trading_pair = f"{local_base_asset}-{local_quote_asset}"
        local_exchange_trading_pair = f"{local_base_asset}{local_quote_asset}"

        local_data_source = BitgetAPIOrderBookDataSource(
            ...
            "args": [
                {
                    "instType": CONSTANTS.INSTRUMENT_TYPE_SPOT_PUBLIC_CHANNEL,
                    "channel": CONSTANTS.WS_ORDER_BOOK_EVENTS_TOPIC,
                    "instId": local_exchange_trading_pair
                },
                {
                    "instType": CONSTANTS.INSTRUMENT_TYPE_SPOT_PUBLIC_CHANNEL,
